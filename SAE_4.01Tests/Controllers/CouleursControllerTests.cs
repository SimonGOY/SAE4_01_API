...
namespace SAE_4._01.Controllers.Tests
{
    ...
    public class CouleursControllerTests
    {
        ...
        [TestMethod]
        public void Moq_PutCouleurTest()
        {
            // Arrange
            mockRepository.Setup(x => x.GetByIdAsync(1).Result).Returns(couleur);
            var init = controller_mock.GetCouleur(1).Result;
            init.Value.PrixCouleur = 9.99;


            // Act
            var res = controller_mock.PutCouleur(couleur.IdCouleur, init.Value).Result;
            var maj = controller_mock.GetCouleur(1).Result;
